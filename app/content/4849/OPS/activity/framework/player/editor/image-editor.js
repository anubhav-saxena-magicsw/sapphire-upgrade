...
define(['marionette', 'player/editor/base-editor'], function(Marionette, BaseEditor) {
	 ...
	 * @access private
	 */
	ImageEditor.prototype.getComponent = function(compData, isSimComp) {
	 ...
	 * @access private
	 */
	ImageEditor.prototype.createCompData = function(compData) {
	...
	ImageEditor.prototype.checkSource = function(objData) {
		objData.src = this.objPathUpdater.getValidatedPath(this.regionId.classRef.editorController.regionId, objData.src);
		if (!objData.rootPath && this.objPathUpdater) {
			objData.rootPath = this.objPathUpdater.rootPathDict[this.regionId.classRef.editorController.regionId];
		}
	};
	/**
	 ...
	 * @access private
	 */
	ImageEditor.prototype.onComponentCreationComplete = function(objComponent) {
		this.objComp = objComponent;
		this.objComp.defaultData = this.defaultCompData;
		this.componentCreated();
	};

	ImageEditor.prototype.updateComponent = function(objComponent, objData) {
		var strPropertyName = objData.actionKeys[objData.actionKeys.length - 1];
		if (strPropertyName === "src") {
			objData.data[strPropertyName] = this.objPathUpdater.getValidatedPath(this.regionId.classRef.editorController.regionId, objData.data[strPropertyName]);
			objData.data.rootPath = this.objPathUpdater.rootPathDict[this.regionId.classRef.editorController.regionId];
		}
		objComponent.model.set(strPropertyName, objData.data[strPropertyName]);
		//objComponent.render();
		if (strPropertyName === "src") {
			objComponent._changeSource(objData.data[strPropertyName]);
		}
	...
	ImageEditor.prototype.destroy = function() {
		return ImageEditor.__super__.destroy(true, this);
	};
