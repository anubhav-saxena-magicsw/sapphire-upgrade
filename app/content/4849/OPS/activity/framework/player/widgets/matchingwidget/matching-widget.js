...
( function($) {"use strict";
		$.widget("custom.MatchingWidget", {
			...
			attachListeners : function() {
				var mapping;
				//apply click events on target
				mapping = "[type=target]";
				$(this.element).find(mapping).unbind("click").bind("click", this, this.onItemClicked);

				//apply click events on source
				mapping = "[type=source]";
				$(this.element).find(mapping).unbind("click").bind("click", this, this.onItemClicked);
			},
			...
			removeListeners : function() {
				var mapping;
				//remove click events on target
				mapping = "[type=target]";
				$(this.element).find(mapping).unbind("click", this, this.onItemClicked);

				//remove click events on source
				mapping = "[type=source]";
				$(this.element).find(mapping).unbind("click", this, this.onItemClicked);
			},
			...
			onItemClicked : function(event) {
				...
				if (type === "source") {
					...
					if (objThis.isTargetOccupied(event.currentTarget)) {
			...
			isSourceSelected : function() {
				if ($(this.options.previousSource).hasClass('activeState')) {
					return true;
				}
				return false;
			},
			...
			removeActiveState : function(element) {
				...
				element.removeClass("activeState");
			},
			 ...
			 */
			evaluate : function(bRemoveIncorrectLines) {
			 ...
			 */
			reset : function() {
				var arrCorrectResponse,iCorrectResponseLen, i, j, mapping, sourceId, targetId, source, arrTarget;
				arrCorrectResponse = this.options.correctMapping;
				iCorrectResponseLen = arrCorrectResponse.length;
				$("#svgelem").empty();
				/*jslint plusplus: true*/
				for ( i = 0; i < iCorrectResponseLen; i++) {
					mapping = arrCorrectResponse[i];
					sourceId = mapping.split(':')[0];
					source = $('#'+sourceId).attr('target',null);
